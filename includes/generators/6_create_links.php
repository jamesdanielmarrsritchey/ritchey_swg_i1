...
# Meta
// Name: Create Links
// Description: Imports the information stored in "/content_files/links/", and uses it to generate those files in the public directory.
# Content
## Prep
$n = 1;
...
$files = ritchey_list_files_with_prefix_postfix_i1_v1("{$location}/content_files/links", NULL, '.txt', TRUE);
foreach ($files as &$item){
	...
	require_once $location . '/dependencies/ritchey_get_line_by_prefix_i1_v3/ritchey_get_line_by_prefix_i1_v3.php';
	$icon = ritchey_get_line_by_prefix_i1_v3($item, 'Icon: ', FALSE, FALSE, TRUE);
	$content_1 = <<<HEREDOC
<div class='section_outter no_print' id='content_1_outter'>
	<div class='section_inner' id='content_1_inner'>
		<h1>{$post_name}</h1>
		<img class='link_icon' src="{$global_configuration_info['address']}{$icon}">
		<div class='link_content'><a href="{$value}">{$value}</a></div>
	</div>
</div>
HEREDOC;
	require_once $location . '/dependencies/ritchey_get_line_by_prefix_i1_v3/ritchey_get_line_by_prefix_i1_v3.php';
